                                ...
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu">
                                            <button class="dropdown-item" data-toggle="modal" data-target="#edit<?= $value->id_stok ?>"><i class="fas fa-edit"></i> Edit</button>
                                            <a class="dropdown-item text-danger hapus" href="<?= base_url('gudang/stok/delete/'.$value->id_stok) ?>"><i class="fas fa-trash"></i> Hapus</a>
                                        </div>
                                    </div>
                                </td>
							</tr>
  ...
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url('gudang/stok/add') ?>" method="post" enctype="multipart/form-data">
        	...
        	<div class="form-group">
                ...
                <?= form_error('jumlah', '<div class="text-danger small ml-3">', '</div>') ?>
        	</div>
        </div>
...
<div class="modal fade" id="edit<?= $value->id_stok ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url('gudang/stok/edit/'.$value->id_stok) ?>" method="post">
            <div class="form-group">
                <label>Nama Barang</label>
                <select name="id_barang" class="form-control" required>
                    <option value="">Pilih Barang</option>
                    <?php foreach ($barang as $b): ?>
                        <option value="<?= $b->id_barang ?>" <?= ($b->id_barang == $value->id_barang) ? 'selected' : '' ?>>
                            <?= $b->nm_barang ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                <label>Jumlah Stok</label>
                <input type="number" name="jumlah" class="form-control" value="<?= $value->jumlah ?>" required>
            </div>
      </div>
